...
| # | Category | 2021 Rank | 2025 Rank |
|---|-----------|-----------|-----------|
| A01 | Broken Access Control | #1 | #1 |
| A02 | Security Misconfiguration | #5 | #2 |
| A03 | Software Supply Chain Failures | #6 (Vulnerable Components) | #3 |
| A04 | Cryptographic Failures | #2 | #4 |
| A05 | Injection | #3 | #5 |
| A06 | Insecure Design | #4 | #6 |
| A07 | Authentication Failures | #7 | #7 |
| A08 | Software or Data Integrity Failures | #8 | #8 |
| A09 | Security Logging & Alerting Failures | #9 | #9 |
| A10 | Mishandling of Exceptional Conditions | NEW | #10 |
...
**Broken Access Control happens when a system doesn’t correctly enforce rules about who can access or do what.**

**Example:**
An employee who doesn’t work in HR can still see salary and payroll information, even though only HR staff should have that access.

**Vulnerabilities Falls Under This Category**

**1. IDOR (Insecure Direct Object Reference)** is a type of security flaw where an application lets users access data just by changing a reference (like an ID) in a URL or request—without checking if they are allowed to see it.
```
...
```
Here are the common ways IDOR is performed:

1. URL Parameter Manipulation
      Changing an ID in the URL
      Example: /profile?id=101 → change to id=102

2. API Requests (JSON / REST APIs)
...
3. Cookies or Session Data
      Changing stored IDs in cookies (if not protected)
      Example: user_id=101 → change to 102

4. File Access
      Accessing files by changing file names or IDs
      Example:
      /files/invoice_101.pdf → try invoice_102.pdf
```
**Impact of IDOR Vulnerability**
```
1. Read-Only Access (Low Severity)
- If a user changes the ID value and can only view/read data (but cannot edit or delete it), then it is usually a low severity IDOR.

Example: Changing id=101 to id=102 lets you see another user’s profile, but you cannot modify anything.

2. Modify Other User’s Data → Horizontal Privilege Escalation
- If an attacker can modify or update another user’s data (like profile details) without proper checks, then it becomes more serious. This leads to Horizontal Privilege Escalation.

- Why horizontal?
Because the attacker is accessing or modifying data of another user with the same privilege level (user → user).

- Example: A normal user edits another user’s profile (name, email, etc.) by changing the ID.

3. Access or Modify Admin Data → Vertical Privilege Escalation
- If an attacker can access or modify admin-level data, then it becomes critical. This leads to Vertical Privilege Escalation.

- Why vertical?
Because the attacker is moving from a lower privilege (user) to a higher privilege (admin).

- Example: A normal user changes the ID and is able to access or update an admin profile or admin settings.
```


**2. Force Browsing (or Forced Browsing)** is a security issue where a user can access hidden or restricted pages just by typing or guessing the URL directly, instead of going through the proper navigation or login checks.
```
https://app.com/admin/dashboard    # Regular user types this URL directly
```

**3. CORS Misconfiguration** — When a website allows requests from untrusted origins (domains).

  1. **What is CORS** - CORS (Cross-Origin Resource Sharing) is a rule that controls which websites are allowed to access data from another site.
      
  2. **What is CORS Misconfiguration** - CORS misconfiguration happens when a website allows the wrong or untrusted sites to access its data.
      
**CORS Misconfiguration Example (Step by Step):** When a user (or attacker) changes or adds the Origin header in a request and sends it to the server, the server responds with CORS headers. By checking these headers, we can see if CORS is misconfigured.
      
  **Cases:**
```
1. Access-Control-Allow-Origin: *
    → Misconfigured, but usually no serious impact (because credentials like cookies are not allowed).

2. Access-Control-Allow-Origin: http://evil.com
    → Misconfigured
    → The server is trusting an untrusted (malicious) domain.

3. Access-Control-Allow-Origin: https://evil.com and Access-Control-Allow-Credentials: true
    → Misconfigured and Exploitable
    → This allows the attacker’s site to send requests with user credentials (cookies, sessions) and read sensitive data.
```

**Privilege Escalation** — Privilege escalation happens when a normal user becomes an admin or gets more control than allowed.

Example:
```
1. A normal user logs into a website and changes a request (for example, modifying role=user to role=admin).
Because the server doesn’t properly check permissions, the user becomes an admin and can access restricted features like managing users or viewing sensitive data.

2. A user accesses /user/settings and then manually tries /admin/settings in the URL—and it works without proper checks.
```
### 🛡️ How to Prevent
...
Security Misconfiguration happens when a system is not properly configured, leaving it open to attacks.

**Vulnerabilities Falls Under This Category**

**1. Default Credentials** — Using default usernames/passwords (e.g., admin/admin)
```
# Real world: Thousands of IoT devices use default "admin/admin" login
http://router.local/admin  →  Login: admin / Password: admin  ❌
```

**2. Verbose Error Messages / Stack Traces** — Server reveals internal code structure to attackers.
```
# What the user sees in a misconfigured app:
- Showing stack traces or system details to users
- Revealing database queries or server paths
...
```
<img width="719" height="300" alt="image" src="https://github.com/user-attachments/assets/3bd56b16-478b-43af-8053-919fae657c05" />
<br>

**3. Directory Listing Enabled** — Anyone can browse your server's file structure like an open folder.
```
When directory listing is enabled, visiting a URL like: https://example.com/uploads/
might show a page listing all files inside that folder instead of blocking access.
```
<img width="719" height="300" alt="image" src="https://github.com/user-attachments/assets/ad28a025-0c95-43e7-b410-b47c58861e9e" /><br>
<img width="719" height="300" alt="image" src="https://github.com/user-attachments/assets/9a1b598b-629b-4444-a371-de4a68b574d1" />

**IMPACT**
```
Exposing directory contents can unintentionally reveal:
📄 Sensitive files (backups, configs, logs)
🧩 Application structure (helps attackers map your system)
```
**How to prevent it**
```
1. Disable directory listing on your server
Apache → turn off Options Indexes
Nginx → set autoindex off

2. Place an index.html or similar file in directories to prevent listing.
3. Store sensitive files outside the web root
```

**4. Unnecessary Features Enabled** — Test endpoints, debug modes, or unused ports left open in production.
